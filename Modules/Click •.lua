...
eventLoop = function()
	...
	if new <= 0 then
		if timer > 0 then
			timer = timer - .5
			ui.setMapName("CLICK!<")
			ui.addTextArea(0,"<p align='center'><font size='30'>Click as fast as you can! You have "..math.floor(timer).." seconds!",nil,5,30,nil,nil,1,1,0,true)
		...
		ui.setMapName("<J>New game in <ROSE>"..math.floor(new).." <J>seconds!<")
	end
end

eventTextAreaCallback = function(i,n,c)
	if c == "." then
		if new <= 0 and not tfm.get.room.playerList[n].isDead then
			info[n].count = info[n].count + 1
